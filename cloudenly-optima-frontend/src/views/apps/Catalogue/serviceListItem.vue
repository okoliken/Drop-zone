<template>
  <div class="bg-white">
    <s-modal :modal="deactivateModal">
      <div style="width: 450px" class="py-2 px-2">
        <p class="text-lg font-bold">Deactivate Item</p>

        <div class="mb-5 mt-8">
          <p class="font-normal">
            Are you sure you want to deactivate this service Item?
          </p>
        </div>
        <hr class="mt-12 text-gray-400" />
        <div class="flex items-center justify-between mt-8 mb-3">
          <button @click="deactivateModal = false" class="py-4 px-8 font-bold bg-gray-300 rounded-full">
            Cancel
          </button>

          <s-button :loading="deactivateButton" @click="deactivateServiceItems"
            class="py-4 px-8 font-bold bg-primary-optima-color text-white rounded-full">
            Proceed
          </s-button>
        </div>
      </div>
    ...
    <s-modal :modal="deleteModal">
      <div style="width: 450px" class="py-2 px-2">
        <h5 class="text-lg font-bold">Delete Item</h5>

        <div class="mb-5 mt-8">
          <p class="font-normal">Are you sure you want to delete this service Item?</p>
        </div>
        <hr class="mt-12" />

        <div class="flex items-center justify-between mt-5">
          <button @click="deleteModal = false" class="py-4 px-8 font-bold bg-gray-300 rounded-full">
            Cancel
          </button>

          <s-button :loading="deleteButton" @click="deleteServiceItem"
            class="py-4 px-8 font-bold bg-primary-optima-color text-white rounded-full">
            Proceed
          </s-button>
        </div>
    ...
    <div class="py-5">
      <div v-if="getIsAccOwner || getSalesCataloguePrivileges.viewItemRegister" class="mt-3">
        <div v-if="filterItem.length > 0" class="bg-white">
          <div class="flex items-center justify-between w-full px-3 mt-2 mb-2">
            <div class="flex items-center w-2/3">
              <div class="mr-3 w-1/2">
                <select v-model="outletOrOrg" @change="getOutletOrOrgServiceItem"
                  class="w-full py-2 border border-1 border-gray-400 rounded-lg">
                  <option selected value="all">All</option>
                  <option :value="outlet.id" v-for="(outlet, index) in getAllOrganisationLocation.outlets" :key="index">
                    {{ outlet.name }}
                  </option>
                </select>
              </div>

              <div class="1/4 flex items-center border border-1 rounded-lg border-gray-400 px-2">
                <svg class="mx-4" width="16" height="17" viewBox="0 0 16 17" fill="none"
                ...
                <svg class="mr-3" width="1" height="37" viewBox="0 0 1 37" fill="none"
                  xmlns="http://www.w3.org/2000/svg">
                  <line x1="0.25" y1="0.75" x2="0.249999" y2="36.25" stroke="#9BA0A7" stroke-width="0.5"
                    stroke-linecap="round" />
                </svg>
                <input class="p-0 border-0 m-0" placeholder="*To Date" type="date" name="date" id="" />
              </div>
              <div class="ml-3 w-1/4">
                <select class="py-2 border border-1 border-gray-400 rounded-lg" name="" id="">
            ...
            <div class="flex items-center">
              <svg width="1" height="19" viewBox="0 0 1 19" fill="none" xmlns="http://www.w3.org/2000/svg">
                <line x1="0.25" y1="0.25" x2="0.249999" y2="18.75" stroke="#9BA0A7" stroke-width="0.5"
                  stroke-linecap="round" />
              </svg>
          ...
          <table class="table min-w-full">
            <thead class="bg-gray-300">
              <tr class>
                <th class="py-4 px-6 tracking-wider text-dark-optima-color">Name</th>
                <th class="py-4 px-6 tracking-wider text-dark-optima-color">Category</th>

                <th class="py-4 px-6 tracking-wider text-dark-optima-color">
                  Sub Category
                </th>

                <th class="py-4 px-6 tracking-wider text-dark-optima-color">Unit</th>
            ...
            <tbody>
              <tr v-for="(item, index) in filterItem" :key="index"
                class="cursor-pointer hover:bg-grey-lighter border border-solid border-grey-lighter font-normal">
                <td class="px-6 py-3 whitespace-no-wrap border border-gray-200">
                  <div class="flex items-center">
                    <div v-if="item.imageUrl !== null" class="mr-5">
                      <img :src="item.imageUrl" alt="" srcset="" class="h-12 w-12 img-cover rounded-full" />
                    </div>
                ...
                <td class="px-6 py-3 whitespace-no-wrap border border-gray-200">
                  <div class="flex">
                    <div class="mt-4">{{ item.category }}</div>
                  </div>
                </td>

                <td class="px-6 py-3 whitespace-no-wrap border border-gray-200">
                  <div class="flex">
                    <div class="mt-4">{{ item.subCategory }}</div>
                  </div>
                </td>

                <td class="px-6 py-3 whitespace-no-wrap border border-gray-200">
                  <div class="flex">
                    <div class="mt-4">{{ item.unitOfService }}</div>
                  </div>
                </td>

                <td class="px-6 py-3 whitespace-no-wrap border border-gray-200">
                  <div class="flex">
                    <div class="mt-4">{{ item.updatedAt | date }}</div>
                  </div>
                </td>
                    ...
                    <div slot="body">
                      <div class="p-2 bg-white">
                        <ul style="min-width: 12rem">
                          ...
                          <li @click="toggleDeactivateModal(item)"
                            class="flex py-2 px-4 cursor-pointer hover:bg-primary hover:text-white">
                            Deactivate
                          </li>
        ...
        <div v-else class="flex items-center justify-center mx-auto py-20">
          ...
          <div class="">
            <p class="font-semibold mb-3 text-primary-optima-color">
              There are no service catalogue at the moment
            </p>
            <p class="mb-3">Create new or import with the options below.</p>

            <div class="items-center justify-between">
              <button @click.prevent="routeCreateServiceItem"
                class="mr-8 py-5 px-10 rounded-full bg-primary-optima-color text-white" href="#">
                Create New
              </button>

              <button class="py-5 px-10 rounded-full bg-gray-200 text-primary-optima-color"
                @click.prevent="routeImportCatalogue" href="#">
                Import Service
              </button>
            </div>
          </div>
...
export default {
  ...
  computed: {
    ...
    ...mapGetters("auth/roles", ["getIsAccOwner", "getSalesCataloguePrivileges"]),
    ...mapGetters("catalogue/serviceItem", ["getServiceItem"]),
    ...mapGetters("userOutlet", ["getAllOrganisationLocation"]),

    filterItem() {
      return this.getServiceItem.filter((resp) => {
  ...
  methods: {
    ...
    toggleDeactivateModal(val) {
      console.log(val);
      this.deactivateModal = true;
      this.variantItem = {
        orgId: val.orgId,
        outletId: val.feeInformation.outletId,
        serviceId: val.id,
      };
    },
    ...
    deactivateServiceItems() {
      ...
      this.$store
        .dispatch("catalogue/serviceItem/deactivateServiceItemAction", this.variantItem)
        .then((resp) => {
    ...
    deleteServiceItem() {
      ...
      this.$store
        .dispatch("catalogue/serviceItem/deleteServiceItemAction", this.variantItem)
        .then((resp) => {
    ...
    getOutletOrOrgServiceItem() {
      if (this.outletOrOrg === "all") {
        this.$store.dispatch("catalogue/serviceItem/getItemServiceActionOrganization");
      } else {
        this.$store.dispatch(
          "catalogue/serviceItem/getItemServiceActionOutlet",
          this.outletOrOrg
        );
      }
    },
    edit(item) {
      console.log(item.id);
      // this.$router.push()
      // this.$store.dispatch("catalogue/productItem/updateServiceItemAction")
    },
  },

  async mounted() {
    this.outletOrOrg = this.$store.state.auth.auth.authData.outletId;
    this.$store.dispatch(
      "catalogue/serviceItem/getItemServiceActionOutlet",
      this.outletOrOrg
    );
