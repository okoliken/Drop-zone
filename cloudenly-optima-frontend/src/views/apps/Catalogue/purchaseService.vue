<template>
  <div>
    ...
    <vs-popup
      button-close-hidden
      title="Update Main Variant"
      :active.sync="editMainVariantModal"
    >
      <div>
        <div class="form-group row">
          <label for class="col-4 col-form-label">
            <p class="text-xl font-normal">Name :</p>
          </label>
          <div class="col-7">
            <input
              type="text"
        ...
        <div class="form-group row">
          <label for class="col-4 col-form-label">
            <p class="text-xl font-normal">Brand :</p>
          </label>
          <div class="col-7">
            <select
              v-model="mainVariant.brand"
              class="py-2 px-2 border border-solid border-grey rounded w-full"
            >
              <option
                v-for="(brand, index) in getallBrand"
        ...
        <div class="form-group row">
          <label for class="col-4 col-form-label">
            <p class="text-xl font-normal">Category :</p>
          </label>
          <div class="col-7">
            <select
              v-model="mainVariant.categoryId"
              class="py-2 px-2 border border-solid border-grey rounded w-full"
            >
              <option selected value>Select or Create New</option>
              <option
                v-for="(cat, index) in getallItemCategory"
                :key="index"
                :value="cat.id"
        ...
        <div class="vx-col w-full md:w-1/3 lg:w-1/3 xl:w-1/3">
          <div class="flex flex-end justify-end">
            ...
            <!-- <vs-dropdown vs-custom-content vs-trigger-click>
              <button class="inline-flex items-center py-2 px-4 rounded-full bg-gray-100">
                <span class="mr-2">Import/Export</span>
                <feather-icon icon="ChevronDownIcon" svgClasses="h-6 w-6 mr-1"></feather-icon>
              </button>
            ...
            <button
              v-if="
                getIsAccOwner || getPurchaseCataloguePrivileges.exportCatalogue
              "
              @click.prevent="exportCatalogueModal = true"
              class="
                inline-flex
              ...
              "
            >
              <span class="">Export Catalogue</span>
            </button>
            ...
            <!-- <vs-dropdown vs-custom-content vs-trigger-click>
              <button
                class="inline-flex items-center    
                 border-0 py-1 mr-2 px-2 cursor-pointer   bg-cloudenly-primary-main text-white rounded"
              >
              ...
              <vs-dropdown-menu>
                <ul style="min-width: 15rem">
                  ...
                  <li
                    class="flex py-2 px-4 cursor-pointer hover:bg-primary hover:text-white"
                  >
                    <span class="ml-2"> From Sales Catalogue</span>
                  </li>

                  <li
                    class="flex py-2 px-4 cursor-pointer hover:bg-primary hover:text-white"
                  >
                    <span class="ml-2"> From Inventory Catalogue</span>
                  </li>
                </ul>
              </vs-dropdown-menu>
            </vs-dropdown>-->
    ...
    <div
      v-if="getIsAccOwner || getPurchaseCataloguePrivileges.viewItemRegister"
    >
      <div class="mt-8" v-if="getallPurchaseService.length > 0">
        <div class="bg-white">
          <table class="table min-w-full">
            ...
            <tbody>
              <tr
                v-for="(item, index) in getallPurchaseService"
                :key="index"
                class="
                  cursor-pointer
              ...
              >
                ...
                <vs-checkbox :vs-value="item" v-model="items" class="pl-1"></vs-checkbox>
              </td> -->
                ...
                <td
                  ...
                  class="px-6 py-3 whitespace-no-wrap border border-gray-200"
                >
                  <span v-if="item.category === null">N/A</span>
                ...
                >
                  <vs-dropdown vs-custom-content vs-trigger-click>
                    ...
                    <vs-dropdown-menu>
                      <ul style="min-width: 10rem">
                        <!-- <li
                        @click="editMainVariant(item)"
                        class="flex py-2 px-4 cursor-pointer hover:bg-primary hover:text-white"
                      >
                        <span>Edit Main Variant</span>
                      </li> -->
                        <li
                          @click="routeCreateVariant(item)"
                          class="
                            flex
                            py-2
                            px-4
...
export default {
  ...
  computed: {
    ...
    filterService() {
      return this.getallPurchaseService.filter((resp) => {
  ...
  methods: {
    ...
    selectAll() {
      this.items = [];
      if (!this.isSelected) {
        this.getallPurchaseProduct.map((item) => {
  ...
  mounted() {
    this.$store.dispatch("catalogue/getAllPurchaseService");
