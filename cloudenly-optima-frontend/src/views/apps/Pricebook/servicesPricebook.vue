<template>
  <div>
    <div v-if="getServicePricebook.length > 0">
              ...
              <tbody>
                <tr
                  class="
                    ...
                    border-l-0
                  "
                  v-for="(pricebook, index) in getServicePricebook"
                  :key="index"
                >
                  <td
...
import emptyState from "./emptyStateService.vue";
...
export default {
  ...
  methods: {
    ...
    getPricebookPriceType() {
      ...
      if (this.priceType === "retail") {
        this.$store
          .dispatch(
            "catalogue/pricebook/getAllServiceRetailPricebook",
            "service"
          )
          .then((resp) => {
            if (resp.data) {
        ...
        this.$store
          .dispatch(
            "catalogue/pricebook/getAllServiceWholesalePricebook",
            "service"
          )
          .then((resp) => {
            if (resp.data) {
  ...
  computed: {
    ...mapGetters("catalogue/pricebook", ["getServicePricebook"]),
  },
