<template>
  <div class>
    <div v-if="getProductPricebook.length > 0">
      <div class="mt-5 bg-white">
        <div class="py-5 px-3 flex items-center justify-between w-full">
          <div class="flex items-center">
            <s-combo-box class="mr-3" />
            <s-select
              @change="getPricebookPriceType"
              v-model="priceType"
              :options="priceList"
            />
          </div>
          <div>
            <div class="flex items-center mx-5">
              <button
                ...
                "
              >
                <span class="text-primary-optima-color mr-3"> Export</span>
              </button>
      ...
      <div
        id="div-with-loading"
        class="vs-con-loading__container bg-white mt-3"
      >
        <div>
          <div v-if="priceType === 'retail'">
            <table class="table min-w-full">
              ...
              <tbody>
                <tr
                  class="font-normal"
                  v-for="(pricebook, index) in getProductPricebook"
                  :key="index"
                >
                  <td
                    class="px-6 py-4 whitespace-no-wrap border border-gray-200"
                  >
                    {{ pricebook.variant.variantName }}
                  </td>

                  <td
                    class="px-6 py-4 whitespace-no-wrap border border-gray-200"
                  >
                    {{ pricebook.unitOfSale }}
                  </td>
                  ...
                  >
                    <span>{{
                      pricebook.recommendedRetailPrice | currency
                    }}</span>
                  </td>
                  <td
                    class="px-6 py-4 whitespace-no-wrap border border-gray-200"
                  >
                    {{ pricebook.variant.retailDiscountLimit }}
                  </td>

                  <td
                    class="px-6 py-4 whitespace-no-wrap border border-gray-200"
                  >
                    {{ pricebook.variant.updatedAt | date }}
                  </td>
                </tr>
              </tbody>
            </table>
              ...
              <tbody>
                ...
                >
                  ...
                  <td
                    class="px-6 py-4 whitespace-no-wrap border border-gray-200"
                  >
                    <span>{{ pricebook.unitOfSale }}</span>
                  </td>
                  <td
                    class="px-6 py-4 whitespace-no-wrap border border-gray-200"
                  >
                    <span>{{
                      pricebook.recommendedWholesalePrice | currency
                    }}</span>
                  </td>
                  <td
                    class="px-6 py-4 whitespace-no-wrap border border-gray-200"
                  >
                    {{ pricebook.variant.wholesaleDiscountLimit }}
                  </td>
...
export default {
  ...
  methods: {
    ...
    getPricebookPriceType() {
      ...
      if (this.priceType === "retail") {
        this.$store
          .dispatch(
            "catalogue/pricebook/getAllProductRetailPricebook",
            "product"
          )
          .then((resp) => {
            if (resp.data) {
        ...
        this.$store
          .dispatch(
            "catalogue/pricebook/getAllProductWholesalePricebook",
            "product"
          )
          .then((resp) => {
            if (resp.data) {
  ...
  computed: {
    ...mapGetters("catalogue/pricebook", ["getProductPricebook"]),
  },
