        ...
        >
          ...
          <div>
            ...
            <select
              ...
              class="w-full py-2 border-2 border-solid rounded"
              name
              id
            >
                  ...
                  >
                    ...
                    >
                      <input
                        v-model="boq.name"
                        type="text"
                        class="py-2 px-2 border-2 border-solid border-grey w-full rounded"
                      />
                    </td>
                    <td
                      class="px-3 text-center py-3 whitespace-no-wrap border-b border-gray-200"
                    >
                      <select
                        ...
                        class="py-1 px-2 border-2 border-solid border-grey w-full rounded"
                        name
                        id
                      >
                        <option
        ...
        <div class="flex justify-between">
          ...
          <div>
            <button
              ...
              type=" button"
              class="mr-5 px-5 rounded-full flex items-center py-3 text-white bg-primary-optima-color"
            >
...
export default {
  ...
  methods: {
    ...
    matchItem(val, index) {
      // console.log(val, index, val.itemMatched);
      ...
      this.selectedItem.forEach((resp, i) => {
        if (index === i) {
          ...
          (val.invoiceTaxPolicyRates = val.itemMatched.item.taxes.map(
            (resp) => {
              return {
                name: resp.name,
                amount: resp.amount,
        ...
        case "taxExclusive":
          ...
          val.invoiceBoqTaxes = val.invoiceTaxPolicyRates.map((resp) => {
            return {
              ...
              rate: resp.amount,
              amount:
                (Number.parseFloat(resp.amount) * Number.parseFloat(val.discountedLineTotal)) / 100,
            };
          });
      ...
      switch (shippingChargeTaxPolicy) {
        ...
        case "taxExclusive":
          ...
          this.invoiceData.totalTaxOnShippingCharge =
            (Number.parseFloat(this.invoiceData.shippingCharge) * exclusiveTotalTaxes) /
            100;
          this.invoiceData.invoiceShippingChargeTaxes = this.allVat.taxes.map(
            (resp) => {
              return {
                ...
                rate: resp.amount,
                amount:
                  (Number.parseFloat(resp.amount) * this.invoiceData.shippingCharge) / 100,
              };
            }
          );
          break;
      ...
      switch (specialDiscountTaxPolicy) {
        ...
        case "taxExclusive":
          ...
          this.invoiceData.totalTaxOnSpecialDiscount =
            (Number.parseFloat(this.invoiceData.specialDiscount) * exclusiveTotalTaxes) /
            100;
          this.invoiceData.invoiceSpecialDiscountTaxes = this.allVat.taxes.map(
            (resp) => {
              return {
                ...
                rate: resp.amount,
                amount:
                  (Number.parseFloat(resp.amount) * this.invoiceData.specialDiscount) /
                  100,
              };
            }
          );
  ...
  computed: {
    ...
    calTotalSubLineTotalTaxAmount() {
      ...
      for (var i = 0; i < this.selectedItem.length; i++) {
        totalSubLineTotalTaxAmount =
          totalSubLineTotalTaxAmount + Number.parseFloat(this.selectedItem[i].totalTax);
      }
      // eslint-disable-next-line vue/no-side-effects-in-computed-properties
    ...
    calSalesInvoiceTotal() {
      ...
      return (this.invoiceData.invoiceTotal =
        Number.parseFloat(total) + Number.parseFloat(this.invoiceData.salesTaxTotal));
      // eslint-disable-next-line vue/no-side-effects-in-computed-properties
    ...
    calTotalCustomerDiscount() {
      ...
      this.invoiceData.totalCustomerDiscount =
        Number.parseFloat(this.invoiceData.totalSubLineTotalDiscountAmount | 0) +
        Number.parseFloat(this.invoiceData.specialDiscount | 0);
      // eslint-disable-next-line vue/no-side-effects-in-computed-properties
  ...
  mounted() {
    ...
    this.$store
      .dispatch("sales/SaleOrder/getSingleSaleOrder", id)
      .then((resp) => {
        // console.log(resp.data);
        this.invoiceData.customer = resp.data.senderInfo;
        this.selectedCustomer = resp.data.senderInfo;
        this.invoiceData.currency = resp.data.purchaseOrder.currency;
        this.invoiceData.deliveryAddress = resp.data.purchaseOrder.shippingAddress;
        this.invoiceData.billingAddress = resp.data.purchaseOrder.deliveryAddress;
        this.invoiceData.description = resp.data.purchaseOrder.description;
        this.invoiceData.paymentTerms = resp.data.purchaseOrder.paymentTerms;
        this.invoiceData.originId = resp.data.id;
        this.invoiceData.originObject = "purchaseOrder";
        this.selectedItem = resp.data.purchaseOrder.purchaseOrderBoqs.map(
          (resp) => {
            return {
